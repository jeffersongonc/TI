...
def searchNews(subjectVar, lb_news):
	url_base = 'https://busca.uol.com.br/result.html?term='

	busca = 'greve atibaia'#subjectVar.get()

	resposta = requests.get(url_base + busca)
    
	site = BeautifulSoup(resposta.text, 'html.parser')

	noticias = site.findAll('div', attrs={'class': 'gsc-expansionArea'})
	print(noticias)
'''
	lb_pos = 0
	...
	for noticia in noticias:
		titulo = noticia.find('div', attrs={'class': 'gs-title'}).text

		descricao = noticia.find('div', attrs={'class': 'gs-bidi-start-align'}).text
